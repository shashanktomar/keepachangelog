...
export default function parseChangelog(string) {
  ...
  if (res.status === true) return res.value;
  else throw new Error('Could not parse changelog');
}
...
function innerParser(parser) {
  return PS.co(function*() {
    ...
    if (!res.status) yield P.fail('inner parser failed');
...
function sectionParser(sectionLevel) {
  return PS.co(function*() {
    ...
    if (level !== sectionLevel) yield P.fail();
